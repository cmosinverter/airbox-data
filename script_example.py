...
def correction_all_points_processing(surv_vender: utils.RefSurveiesVender, stat_vender: utils.RefStationsVender, regression_date_list):
    ...
    stat_regression_point_list = stat_vender.getDataByDateList(
        regression_date_list)
    #此為機器按時間的之一小時內所有點, 應取平均或某種取點法
    surv_regression_point_list = surv_vender.getDataByDateList(
        regression_date_list)

    result = []
    for current_date in regression_date_list:
        pass_points = utils.getIndex_of_current_date_and_before(
            current_date, regression_date_list)
        pass_points.reverse()
        ...
        try:
            ...
            point_surv_i = surv_vender.get_value_array_of_na(
                SENSOR_ORDER, surv_selected)
        except utils.MissingValueException:
            continue  # 在這個範例 當前點 有缺漏值就不做校正

        try:
            point_stat_i = stat_vender.get_value_array_of_ppb(
                GAS_ORDER, stat_regression_point_list[point_index_i])
        except utils.MissingValueException:
            continue  # 在這個範例 當前點 有缺漏值就不做校正
        ...
        for index in pass_points[1:3]:  # 在這個範例至多只檢查就近兩點
            try:
                ...
                point_surv_j = surv_vender.get_value_array_of_na(
                    SENSOR_ORDER, surv_selected)
                ...
                continue  # 有缺漏值就下一個
            try:
                point_stat_j = stat_vender.get_value_array_of_ppb(
                    GAS_ORDER, stat_regression_point_list[index])
            except utils.MissingValueException:
                point_surv_j = None
                point_stat_j = None
                continue  # 有缺漏值就下一個
        ...
        a, b = corrected_ec_regression(
            point_surv_i, point_surv_j, point_stat_i, point_stat_j)
...
def ec_regression_all_points_processing(sync_points, surv_vender):
    ...
    def fn(row):
        ...
        between_start = utils.any_datetime_2_utc_timestamp(
            sync_points[current_sync_point_index]["date"])
        if current_sync_point_index+1 < len(sync_points):
            between_end = utils.any_datetime_2_utc_timestamp(
                sync_points[current_sync_point_index+1]["date"])
        else:
            between_end = np.inf
...
if __name__ == '__main__':
    ...
    device_uuid = '04311cdc-a6a5-4d5e-916e-4e75076a8f0a'  # ENSABE020001 (SGX)
    # device_uuid = 'c92494ea-5e44-4507-8776-984705d1fd2b' # ENSABE020002 (SPEC)
    # 設定掃描時間區間與現在時間
    ...
    # 實例化機器提取工具
    surv_vender = utils.RefSurveiesVender(
        surv_storage, device_uuid, now, m_earlier, m_later)
